  ...
  <div class="max-w-4xl mx-auto px-6 py-10">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-14">
      <div v-for="stat in stats" :key="stat.label" class="text-center">
        ...
        <p
          class="text-4xl font-bold text-gray-900 dark:text-white leading-none"
        >
          {{ stat.value }}
        </p>
    ...
    <div class="mb-12">
      <p
        class="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-6"
      >
        ...
        <div v-for="area in lifeAreaBreakdown" :key="area.value">
          <div class="flex items-center justify-between mb-1.5">
            ...
            <span
              ...
              :class="area.pct === 100 ? 'text-green-500' : 'text-gray-400'"
            >
    ...
    <div class="mb-12">
      ...
      <div
        v-if="!store.completedThisYear.length"
        class="text-gray-400 text-sm py-4 italic"
      >
        ...
        >
          ...
          <div class="min-w-0">
            ...
            <p
              ...
              class="text-sm text-gray-600 dark:text-gray-400 mt-1 italic line-clamp-2"
            >
              "{{ item.reflection }}"
            </p>
          ...
          <span class="text-xs text-gray-400 shrink-0 mt-0.5 ml-auto">{{
    ...
    <div v-if="reveals && store.items.length >= 3" class="mb-12">
      ...
      <div
        v-if="reveals.archetype"
        class="mb-6 p-6 rounded-2xl bg-linear-to-br from-primary-50 to-primary-100/50 dark:from-primary-950/30 dark:to-primary-900/10 border border-primary-100 dark:border-primary-900/50"
      >
        <p
          class="text-xs font-semibold uppercase tracking-widest text-primary-400 mb-1"
        >
          Your archetype
        </p>
      ...
      <p class="text-gray-600 dark:text-gray-400 leading-relaxed text-lg">
        ...
        >
          {{ reveals.topCat.label }}</span
        >
        <span v-if="reveals.topCat">
          comes up most — {{ reveals.topCatCount }}
          {{ reveals.topCatCount === 1 ? "time" : "times" }}.</span
        >
        <span v-if="reveals.topArea">
          <span class="font-semibold text-gray-800 dark:text-gray-200 pl-2">{{
            reveals.topArea.label
          }}</span>
          is the life area you keep returning to.</span
        >
        <span v-if="reveals.oldest && reveals.oldestDays > 90">
          Your oldest unfulfilled dream —
          <span class="italic">"{{ reveals.oldest.title }}"</span> — has been
          waiting {{ reveals.oldestDays }} days.</span
        >
      </p>
    </div>
    ...
    <div>
      ...
      <div
        v-if="!store.addedThisYear.length"
        class="text-gray-400 text-sm py-4 italic"
      >
...
const stats = computed(() => [
  ...
  {
    label: "Lived it",
    value: store.byStatus.done.length,
    icon: "i-lucide-check-circle",
    color: "text-green-500",
  },
  {
    label: "In motion",
    value: store.byStatus["in-progress"].length,
    icon: "i-lucide-zap",
    color: "text-blue-400",
  },
  {
    label: "Lived this year",
    value: store.completedThisYear.length,
    icon: "i-lucide-star",
...
const reveals = computed(() => {
  ...
  const catCounts = store.items.reduce(
    (acc, i) => {
  ...
  const areaCounts = store.items.reduce(
    (acc, i) => {
